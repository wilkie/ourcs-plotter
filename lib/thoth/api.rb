class API
  ...
  def self.publish(interface, options = {})
    puts ""
    iface = Interface.find_by_name(interface)
    if iface.nil?
      puts "Interface not found."
      return
    end
    ...
    self.hosts.each_with_index do |host, i|
      ...
      puts "Uploading #{interface}.rb"
      File.open(iface.path, "rb") do |file|
        protocol.publish_interface(interface, file)
      end
    end
  end

  def self.publish_implementation(interface, implementation, options = {})
    puts ""
    impl = Implementation.find_by_interface_and_name(interface, implementation)
    if impl.nil?
      puts "Implementation not found."
      return
    end
    ...
    self.hosts.each_with_index do |host, i|
      ...
      # Upload to every neighbor
      puts "Uploading #{implementation}.rb"
      File.open(impl.path, "rb") do |file|
        protocol.publish_implementation(interface, implementation, file)
      end
    end
  end
end
